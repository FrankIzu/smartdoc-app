...
- **Option C fallback:** If PiP did not activate after ~450 ms, we fall back to the in-app bubble and “In GrabDocs meeting – Tap to return” notification so the user is not left with nothing.
...
- **Back button (or in-app “minimize”)** → In-app **floating bubble** only; user stays in the app and can tap the bubble to return to the meeting. No system PiP.
- **Home / app switch (app backgrounded)** → **System PiP** should show the meeting in a small window; we also trigger native `enterPipForMeeting` (Android) and, if PiP does not activate, fall back to the bubble/notification.
