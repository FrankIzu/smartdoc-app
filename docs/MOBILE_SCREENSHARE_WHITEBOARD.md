...
3. **Run pod install locally (on macOS)**  
   ...
   You should see **Pod installation complete!** with no errors. If you see "Unable to find host target(s) for GrabDocsBroadcastUpload", the extension target is not nested inside the main app target in the Podfile; the plugin’s line-based insertion and explicit `host_targets 'GrabDocs'` should prevent that (CocoaPods 1.12+ may not infer the host when React Native uses abstract targets). On Windows, `ios/` is not generated by prebuild; use EAS Build (or a Mac) to verify.
