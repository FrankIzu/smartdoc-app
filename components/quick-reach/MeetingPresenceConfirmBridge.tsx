...
function InnerPresenceBridge({ enabled, onEnteredRoom, onConnectionStuck }: Props) {
  ...
  useHMSPeerUpdates(
    (...args: unknown[]) => {
      ...
      if (!data?.type || !data?.peer) return;

      // Local PEER_JOINED = full room join (not preview). Remote PEER_JOINED implies we are already in.
      if (data.type === 'PEER_JOINED') {
        fireOnce();
        return;
      }

      // Fallback when PEER_JOINED is missed but we are clearly connected (role/metadata after join).
      if (
        data.peer.isLocal &&
        (data.type === 'ROLE_CHANGED' || data.type === 'METADATA_CHANGED')
      ) {
        fireOnce();
      }
    },
